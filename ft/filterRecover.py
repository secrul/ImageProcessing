...
blurred = np.abs(make_blurred(image, PSF, eps))
...
result = wiener(blurred, PSF, eps)  # 维纳滤波
graph.subplot(233)
...
result = inverse(blurred_noisy, PSF, 0.1 + eps)  # 对添加噪声的图像进行逆滤波
graph.subplot(235)
graph.xlabel("inverse deblurred")
graph.imshow(result)

result = wiener(blurred_noisy, PSF, 0.1 + eps)  # 对添加噪声的图像进行维纳滤波
graph.subplot(236)
graph.xlabel("wiener deblurred(k=0.01)")
graph.imshow(result)
